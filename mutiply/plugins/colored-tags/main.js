...
function adapt$1(W1, W2, XYZ, options = {}) {
  ...
  W2 = getWhite(W2);
  if (!W1 || !W2) {
    throw new TypeError(`缺少白点以进行转换 ${!W1 ? "从" : ""}${!W1 && !W2 ? "/" : ""}${!W2 ? "到" : ""}`);
  }
  if (W1 === W2) {
    return XYZ;
  }
  ...
  if (env.M) {
    ...
    throw new TypeError("目前仅支持白点为D50和D65的Bradford色差转换。" );
  }
}
var \u03B5$4 = 75e-6;
var _ColorSpace = class {
  ...
  to(space, coords) {
    ...
    if (!connectionSpace) {
      throw new Error(`无法在颜色空间 ${this} 和 ${space} 之间转换：未找到连接空间`);
    }
  ...
  static get(space, ...alternatives) {
    ...
    if (argType === "string") {
      ...
      if (!ret) {
        throw new TypeError(`未找到id为 "${space}" 的颜色空间`);
      }
      return ret;
    }
    ...
    throw new TypeError(`${space} 不是有效的颜色空间`);
  }
  /**
  ...
  static resolveCoord(ref, workingSpace) {
    ...
    if (!space) {
      throw new TypeError(`无法解析坐标参考 ${ref}：未指定颜色空间，且不允许相对引用`);
    }
    ...
    throw new TypeError(`在 ${space.name} 中未找到 "${coord}" 坐标。其坐标为： ${Object.keys(space.coords).join(", ")}`);
  }
};
...
function parse(str, { meta } = {}) {
  ...
  if (env.parsed) {
    ...
    if (name === "color") {
      ...
      throw new TypeError(`无法解析颜色(${id})。 ` + (didYouMean || "缺少插件？"));
    } else {
      for (let space of ColorSpace.all) {
  ...
  throw new TypeError(`无法将 ${str} 解析为颜色。缺少插件？`);
}
function getColor(color) {
  if (!color) {
    throw new TypeError("空颜色引用");
  }
  if (isString(color)) {
    color = parse(color);
  }
...
function serialize(color, {
  ...
  if (format.type === "custom") {
    ...
    if (format.serialize) {
      ...
      throw new TypeError(`格式 ${formatId} 仅可用于解析颜色，而非序列化`);
    }
  } else {
...
function contrast(background, foreground, o = {}) {
  ...
  if (!algorithm) {
    ...
    throw new TypeError(`contrast() 函数需要一个对比算法。请指定以下之一： ${algorithms}`);
  }
  background = getColor(background);
  foreground = getColor(foreground);
  ...
  throw new TypeError(`未知的对比算法： ${algorithm}`);
}
function uv(color) {
...
function deltaE(c12, c22, o = {}) {
  ...
  throw new TypeError(`未知的 deltaE 方法： ${method}`);
}
...
var ColoredTagsPluginSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setHeading().setName("调色板").setDesc("选择调色板").addDropdown(
      (dropdown) => dropdown.addOptions({
        ["adaptive-soft" /* ADAPTIVE_SOFT */]: "\u{1F338} 自适应柔和",
        ["adaptive-bright" /* ADAPTIVE_BRIGHT */]: "\u{1F33A} 自适应明亮",
        ["custom" /* CUSTOM */]: "自定义"
    ...
    if (this.plugin.settings.palette.selected === "custom") {
      const customPaletteField = new import_obsidian.Setting(containerEl).setName("自定义调色板").setDesc("").addText((text) => {
      ...
      customPaletteField.descEl.innerHTML = `
				每个 RGB 颜色的格式为 <code>XXXXXX-XXXXXX-XXXXXX</code>.<br/>
				您可以分享最佳颜色调色板，或从 <a href="https://github.com/pfrankov/obsidian-colored-tags/discussions/18">社区</a> 获取一个。
			`.trim();
    }
    new import_obsidian.Setting(containerEl).setName("调色板偏移").setDesc(
    ...
    new import_obsidian.Setting(containerEl).setHeading().setName("\u{1F9BE} 可访问性").setDesc("显示可访问性选项").addToggle(
    ...
    if (this.showAccessibility) {
      new import_obsidian.Setting(containerEl).setName("高文本对比度").setDesc("文本仅使用白色和黑色").addToggle(
    ...
    new import_obsidian.Setting(containerEl).setHeading().setName("\u{1F9EA} 实验性").setDesc(
    ...
    if (this.showExperimental) {
      new import_obsidian.Setting(containerEl).setName("混合颜色").setDesc("这有助于使文本可读").addToggle(
      ...
      new import_obsidian.Setting(containerEl).setName("渐变过渡").addToggle(
      ...
      new import_obsidian.Setting(containerEl).setName("重置配置").setDesc(
        "\u{1F6A8} 所有标签的所有颜色将重新计算，就像插件首次启动一样。需要重启 Obsidian。"
      ).addButton(
        (button) => button.setButtonText("重置").setClass("mod-warning").onClick(async () => {
  ...
  async checkUpdates() {
    try {
      ...
      if (response.tag_name !== this.manifest.version) {
        new import_obsidian2.Notice(`\u2B06\uFE0F 彩色标签：有新版本可用`);
      }
    } catch (error) {
      console.error(error);
    }
